...
class MultimodalTrainer:
    def __init__(self, model, train_loader, val_loader):
        ...
        self.optimizer = torch.optim.AdamW([
            # Text encoder (small model - needs careful tuning)
            {'params': model.text_encoder.parameters(), 'lr': 2e-5},
            # Video encoder (larger model - slightly lower LR)
            {'params': model.video_encoder.parameters(), 'lr': 1e-5},
            # Audio encoder (similar to video)
            {'params': model.audio_encoder.parameters(), 'lr': 1e-5},
            # Fusion and classifiers (needs faster learning)
            {'params': model.fusion_layer.parameters(), 'lr': 3e-4},
            {'params': model.emotion_classifier.parameters(), 'lr': 3e-4},
            {'params': model.sentiment_classifier.parameters(), 'lr': 3e-4}
...
#dataset\train\train_sent_emo.csv
#
